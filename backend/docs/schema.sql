...
CREATE UNIQUE INDEX IF NOT EXISTS idx_expert_moderator_unique
    ON expert(discussion_id) WHERE role = 'moderator';

CREATE INDEX IF NOT EXISTS idx_expert_discussion ON expert(discussion_id);

-- ============================================================
-- 4. Transcript（发言记录）
-- ============================================================
-- 业务规则：
--   插入时需由 Repository 层按 (discussion_id) 分组生成递增 sequence，
--   不能依赖时间戳排序。sequence 在同一 discussion 内严格单调递增。
CREATE TABLE IF NOT EXISTS transcript (
    id            TEXT NOT NULL PRIMARY KEY,               -- UUID v4
    sequence      INTEGER NOT NULL DEFAULT 0,              -- 讨论内递增序号（Repository 层生成）
...
CREATE INDEX IF NOT EXISTS idx_transcript_discussion ON transcript(discussion_id);
CREATE INDEX IF NOT EXISTS idx_transcript_speaker    ON transcript(speaker_id);
CREATE INDEX IF NOT EXISTS idx_transcript_reply_to   ON transcript(reply_to_id);
CREATE INDEX IF NOT EXISTS idx_transcript_sequence   ON transcript(discussion_id, sequence);
...
CREATE INDEX IF NOT EXISTS idx_divergence_discussion ON divergence(discussion_id);

-- ============================================================
-- 7. 触发器：updated_at 自动更新
-- ============================================================
CREATE TRIGGER IF NOT EXISTS trg_user_updated_at AFTER UPDATE ON "user"
BEGIN
    UPDATE "user" SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_discussion_updated_at AFTER UPDATE ON discussion
BEGIN
    UPDATE discussion SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_expert_updated_at AFTER UPDATE ON expert
BEGIN
    UPDATE expert SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_transcript_updated_at AFTER UPDATE ON transcript
BEGIN
    UPDATE transcript SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_consensus_updated_at AFTER UPDATE ON consensus
BEGIN
    UPDATE consensus SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_divergence_updated_at AFTER UPDATE ON divergence
BEGIN
...
-- ============================================================
CREATE TRIGGER IF NOT EXISTS trg_transcript_insert_guard BEFORE INSERT ON transcript
FOR EACH ROW
BEGIN
    SELECT CASE
