"""Repository for the ``consensus`` table."""
...
class ConsensusRepository(BaseRepository):
    """Data access for the ``consensus`` table."""

    # ── create ─────────────────────────────────────────────────

    def create(self, data: dict[str, Any]) -> dict[str, Any]:
        """Insert a consensus record and return it.

        Args:
            data: Column-value mapping. Must include ``id``, ``content``,
                ``discussion_id``, ``source_transcript_ids`` (JSON array
        ...
        """
        columns = [c for c in CONSENSUS_COLUMNS if c in data]
    ...
    def update(
        ...
        """Update a consensus record's content and source transcript IDs.

        ``updated_at`` is automatically refreshed by the DB trigger.

        Args:
            consensus_id: Target consensus ID.
            content: New consensus text.
        ...
        with self._connection_factory() as conn:
            conn.execute(
                ...
                "WHERE id = ?",
                (content, source_transcript_ids, consensus_id),
            )
            return self._fetch_by_id(conn, consensus_id)

    # ── get by discussion ──────────────────────────────────────

    def get_by_discussion(self, discussion_id: str) -> list[dict[str, Any]]:
        """Return all consensus records for a discussion.

        Args:
            discussion_id: Target discussion ID.

        Returns:
            List of consensus row dicts.
        """
        with self._connection_factory() as conn:
            cursor = conn.execute(
    ...
    def _fetch_by_id(
        ...
        """Fetch a consensus row using an existing connection."""
        row = conn.execute(
